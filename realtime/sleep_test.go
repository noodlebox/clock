...
func TestSleep(t *testing.T) {
	...
	if duration < delayadj {
		t.Fatalf("time.Sleep(%s) slept for only %s", delay, duration)
	}
}
...
func BenchmarkStartStop(b *testing.B) {
	benchmark(b, func(n int) {
		timers := make([]*Timer, n)
		for i := 0; i < n; i++ {
			timers[i] = time.AfterFunc(Hour, nil)
		}
...
func TestAfterStop(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		time.AfterFunc(100*Millisecond, func() {})
		t0 := time.NewTimer(50 * Millisecond)
		c1 := make(chan bool, 1)
		t1 := time.AfterFunc(150*Millisecond, func() { c1 <- true })
		c2 := time.After(200 * Millisecond)
		if !t0.Stop() {
...
func testAfterQueuing(delta Duration) error {
	...
	for _, slot := range slots {
		go await(slot, result, time.After(Duration(slot)*delta))
	}
	var order []int
	var times []Time
	for range slots {
...
func TestTimerStopStress(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		go func(i int) {
			timer := time.AfterFunc(2*Second, func() {
...
func TestOverflowSleep(t *testing.T) {
	...
	case <-time.After(25 * Millisecond):
		// OK
	}

	const neg = Duration(-1 << 63)
	time.Sleep(neg) // Returns immediately.
	select {
	case <-time.After(neg):
		// OK
...
func checkZeroPanicString(t *testing.T) {
	// NOTE: it panics, but about a nil pointer instead of a more
...
func TestZeroTimer(t *testing.T) {
	...
	for i := 0; i < 1000000; i++ {
		...
		ti := time.NewTimer(0)
		<-ti.C()
		if diff := time.Since(s); diff > 2*Second {
...
func TestTimerModifiedEarlier(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		deadline := time.NewTimer(10 * Second)
		defer deadline.Stop()
		now := time.Now()
		select {
		case <-timer.C():
			if since := time.Since(now); since > 8*Second {
...
func BenchmarkParallelTimerLatency(b *testing.B) {
	...
	// Note that other than the time.AfterFunc calls this benchmark is measuring it
